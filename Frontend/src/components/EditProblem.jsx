...
const EditProblem = () => {
  ...
  const fetchProblem = async () => {
    try {
      ...
      let subCategory = res.data.subCategory || '';
      let customSubCategory = '';
      let subSubCategory = res.data.subSubCategory || '';
      let customSubSubCategory = '';
      const isWanFirewall = category === "WAN Firewalls" || (category === 'Other' && customCategory === "WAN Firewalls");
      const isAccessPoints = category === "Access Points" || (category === 'Other' && customCategory === "Access Points");
      if (isWanFirewall || isAccessPoints) {
        const subLists = isWanFirewall ? wanFirewallSubCategories : accessPointSubCategories;
        if (!subLists.includes(subCategory)) {
          customSubCategory = subCategory;
          subCategory = 'Other';
        }
        const currentSubSubs = isWanFirewall ? wanSubSubCategories[subCategory] || [] : accessPointSubSubCategories[subCategory] || [];
        if (currentSubSubs.length > 0 && !currentSubSubs.includes(subSubCategory)) {
          customSubSubCategory = subSubCategory;
